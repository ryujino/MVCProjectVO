...
import jinoru.mvc.action.ActionHandler;
...
public class MVCController extends HttpServlet {
    ...
    // processRequest가 처리하도록 작성한다.
    private void processRequest(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {

        // 클라이언트의 요청분석
        // 만약에 요청이 이러면 => http://127.0.0.1/board/list.do
        // getReaquestURI() => /board/list.do               <== 원래대로 mvc.properties 파일 내용 그대로
...
//        String cmd = uri.substring(uri.lastIndexOf("/"));
        ...
        ActionHandler handler = (ActionHandler)MVCInitController.getCommandHandlerMap().get(cmd);

        // 핸들러를 실행하고
        // 그 결과를 출력할 뷰와 뷰 종류를 알아낸다.
        ...
        if (checkView[0].equals("1")) {
            ...
            rd.forward(req, res);                                           // forward액션태그한거랑 같다
